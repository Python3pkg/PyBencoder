...
from pybencoder.benexceptions import BenInvalidInputError, BenInvalidEncoded, BenInvalidMark
...
class PyBencoder(object):
    ...
    def get_bentype_encoder(self, elem):
        ...
        if isinstance(elem, tuple([int, bool])):
            return BenInt()
        if isinstance(elem, tuple([str, memoryview])):
            return BenString()
        if isinstance(elem, tuple([list, tuple])):
            return BenList()
        if isinstance(elem, tuple([dict])):
            return BenDict()
    ...
    def get_bentype_decoder(self, elem):
        ...
        if first == self.BENCODE_INT_START:
            return BenInt()
        if first == self.BENCODE_LIST_START:
            return BenList()
        if first == self.BENCODE_DICT_START:
            return BenDict()
        if first.isdigit():
            return BenString()
...
class BenString(PyBencoder):
    ...
    def encode(self, input_data):
        try:
            ascii_ver = input_data.encode('utf-8').decode('ascii')
    ...
    def encode(self, input_data=[]):
        try:
            ...
            for elem in input_data:
                part_result += self.parent.encode(elem)
